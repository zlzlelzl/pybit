...
while True:
    ...
    for binSize_i in range(len(binSizes)):
        ...
        # if not is_opened:
        if 1:
            # if not is_bought_upper_ubb[binSize_i] and
            if 100000000 < df['volume'][1] + df['volume'][2] + df['volume'][3]:
                try:
                    # 싹 다 고쳐야됨 + 모듈화
        ...
        print(
            f"volume : {df['volume'][1] + df['volume'][2] + df['volume'][3]}")
        time.sleep(1)
