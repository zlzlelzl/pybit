...
while True:
    for binSize_i in range(len(binSizes)):
        ...
        if not is_opened:
            if len(bit.fetch_open_orders()) < 2:
                if not is_bought_upper_ubb[binSize_i] and askPrice > df['ubb'][0]:
                    try:
                        bit.create_order(symbol='XBTUSD', type='limit',
                                         side='sell', amount=amount, price=askPrice)
                        bit.create_order(symbol='XBTUSD', type='StopLimit', params={"stopPx": bidPrice+1},
                                         side='sell', amount=amount, price=bidPrice+2)
                        is_bought_upper_ubb[binSize_i] = True
                ...
                if not is_bought_lower_lbb[binSize_i] and bidPrice < df['lbb'][0]:
                    try:
                        ...
                        bit.create_order(symbol='XBTUSD', type='StopLimit', params={"stopPx": askPrice-1},
                                         side='buy', amount=amount, price=askPrice-2)
            ...
            if is_bought_upper_ubb[binSize_i] or is_bought_lower_lbb[binSize_i]:
                ...
                # 요청가격보다 10 차이날때(20이 본전이라)
                    ...
                    if (is_bought_upper_ubb[binSize_i] and df['ubb'][0] - 10) or \
                            (is_bought_lower_lbb[binSize_i] and df['lbb'][0] + 10):
                        bit.cancel_all_orders()
        ...
        print(
            f"4ubb - askPrice : {int(df['ubb'][0] - askPrice)}, bidPrice - 4lbb : {int(bidPrice - df['lbb'][0])}")
