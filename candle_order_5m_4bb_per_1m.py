...
while True:
    ...
    for binSize_i in range(len(binSizes)):
        ...
        df['6ubb'] = df['avg20'] + 6 * df['stddev']  # 상단밴드
        df['6lbb'] = df['avg20'] - 6 * df['stddev']  # 상단밴드
        print(df['4ubb'][0], df['4lbb'][0])
        ...
        # bit.edit_order()
        # id, symbol, type, side, amount=None, price=None, params={}
        ...
        # 구매 시작
        # if not is_opened:
        #     if not is_bought_upper_ubb[binSize_i] and 1000000 < df['volume'][1] + df['volume'][2]:
        #         try:
        #             # 싹 다 고쳐야됨 + 모듈화
        ...
        # print(f"volume : {df['volume'][1] + df['volume'][2]}")
...
columns = ["buy_sum", "sell_sum"]
volume_df = pd.DataFrame(columns=columns)
format = '%Y-%m-%d %H:%M:%S'

is_buy_twice_called = False
is_sell_twice_called = False
while True:

    buy_dict = {}
    sell_dict = {}
    for orderbook in ws.market_depth():
        if orderbook["side"] == "Sell":
            if sell_dict.get(orderbook["price"]) is None:
                sell_dict[orderbook["price"]] = 0

            sell_dict[orderbook["price"]] += orderbook["size"]
        else:
            if buy_dict.get(orderbook["price"]) is None:
                buy_dict[orderbook["price"]] = 0

            buy_dict[orderbook["price"]] += orderbook["size"]

    buy_price, sell_price = list(buy_dict.items()), list(sell_dict.items())
    buy_price.sort(key=lambda x: -x[0])
    sell_price.sort(key=lambda x: x[0])

    n = 100
    # print(buy_price[:n], sell_price[:n])
    buy_sum = second_sum(buy_price[:n])
    sell_sum = second_sum(sell_price[:n])
    if not is_buy_twice_called and buy_sum >= sell_sum * 2:
        send_message(f"구매 볼륨 2배 알람")
        is_buy_twice_called = True

    if buy_sum >= sell_sum * 3:
        send_message(f"구매 볼륨 3배 알람")

    if not is_sell_twice_called and sell_sum >= buy_sum * 2:
        send_message(f"판매 볼륨 2배 알람")
        is_sell_twice_called = True

    if sell_sum >= buy_sum * 3:
        send_message(f"판매 볼륨 3배 알람")

    if (is_buy_twice_called and buy_sum < sell_sum) or \
            (is_sell_twice_called and buy_sum > sell_sum):
        send_message(f"볼륨 원점")
        is_buy_twice_called = False
        is_sell_twice_called = False

    str_time = datetime.strftime(datetime.now(), format)
    data = {"buy_sum": buy_sum, "sell_sum": sell_sum}
    volume_df.loc[str_time] = data
...
# volume_df.hist(args=volume_df["buy_sum"]-volume_df["sell_sum"])
# volume_df.hist(ax=volume_df.index, by=volume_df["buy_sum"]-volume_df["sell_sum"])
